...
#' @name calc.om
#' @title Calculates the order of magnitude for the relative reference levels
#'     B/Bmsy and F/Fmsy
#' @param rep Result of fit.spict().
#' @param CI Confidence intervals to be used for CI range, e.g. 0.9 for the 90%
#'     confidence intervals. By default (CI = 0.95), the 95% confidence
#'     intervals are used for the CI range.
#' @details The lower, upper values and the CI range are based on the 95%
#'     confidence interval (CI; default).
#' @return Matrix containing the order of magnitude for B/Bmsy and F/Fmsy.
#' @export
calc.om <- function(rep, CI = 0.95){
    if(class(rep) != "spictcls" ||
       !"par.fixed" %in% names(rep)) stop("Please provide an object fitted with 'fit.spict'.")
    blbmsy <- get.par("logBlBmsy", rep, exp=TRUE, CI = CI)
    flfmsy <- get.par("logFlFmsy", rep, exp=TRUE, CI = CI)
    ## 95% CI range
